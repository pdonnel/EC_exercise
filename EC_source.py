...
epsilon0 = 8.854e-12     # vaccum permittivity [F/m]


# Plasma inputs
Ne0 = 1.0e19                  # maximum electron density [m^{-3}]
# maximum of electron temperature [J] (Warning: kB included)
Te0 = 2.0e3 * 1.602e-19
...
omega_b = 7.8e10 * 2 * np.pi  # beam pulsation [Hz]
...
# number of grid points in velocity space (Nv for vperp, 2*Nv for vpar)
Nv = 300
...
def compute_N(theta_loc, P_loc, omega_ce_loc, omega_b_loc):
    """
    @brief Compute the refractive index
    """
...
def compute_theta_res(lambda_loc, P_loc, Omegace_on_omegab_loc):
    """
    @brief Compute the resonant angle
    """
    ...
    tmp2 = Omegace_on_omegab_loc * (1 - P_loc) * lambda2 * \
        np.sqrt((Omegace_on_omegab_loc * (1 - lambda2))
                ** 2 + 4 * P_loc * lambda2)
...
def Compute_Gm_derivatives(zn, m):
    ...
    if (m == 0):
        ...
        d2Gm_dx2 = - (6/zn**4 + 3/zn**2) * np.sinh(zn) + \
            (6/zn**3 + 1/zn) * np.cosh(zn)
    elif (m == 2):
        Gm = (3/zn**3 + 1/zn) * np.sinh(zn) - 3 * np.cosh(zn) / zn**2
        dGm_dx = - (9/zn**4 + 4/zn**2) * np.sinh(zn) + \
            (9/zn**3 + 1/zn) * np.cosh(zn)
        d2Gm_dx2 = (36/zn**5 + 17/zn**3 + 1/zn) * np.sinh(zn) - \
                   (36/zn**4 + 5/zn**2) * np.cosh(zn)
    elif (m == 3):
        Gm = - (15/zn**4 + 6/zn**2) * np.sinh(zn) + \
            (15/zn**3 + 1/zn) * np.cosh(zn)
        ...
        Gm = (105/zn**5 + 45/zn**3 + 1/zn) * np.sinh(zn) - \
            (105/zn**4 + 10/zn**2) * np.cosh(zn)
...
def Compute_usefull_derivatives(xn, yn, m):
    """
    @brief Compute derivatives of the |J_{m+1/2}(zn)|^2 / xn
    """
    if (4 * xn**2 > yn**2):
        ...
        df_dx = 4 * xn * (dGm_dx_m*Gm_p - dGm_dx_p*Gm_m) / \
            (np.pi * (zn_p-zn_m))
        df_dy = 2 * (zn_p*dGm_dx_p*Gm_m - zn_m*dGm_dx_m*Gm_p) / \
            (np.pi * (zn_p-zn_m))
...
def Compute_A_B(omega_b, omega_p, Omega_ce, N0, theta, n_beam):
    ...
    [fx, df_dx, df_dy, df_dydy, df_dxdy] = Compute_usefull_derivatives(
        xn, yn, n_beam)
    [gx, dg_dx, dg_dy, dg_dydy, dg_dxdy] = Compute_usefull_derivatives(
        xn, yn, n_beam+1)
    ...
    BigB = (xn / n_beam)**2 * (2*n_beam + 3) / \
        ((n_beam+1)*(n_beam+2)) * ey**2 * (gx - dg_dydy)
    return BigA, BigB
...
def Compute_Dn_and_Pabs_wrapper(args):
    ...
    [Dn, Power_loc] = Compute_Dn_and_Pabs(
        vpar, vperp, iR, vT_on_c, omega_b, Omega_ce, P_loc, theta0_loc, N0_loc, sigma, E2_loc)
    return Dn, Power_loc
...
def Compute_Dn_and_Pabs(vpar, vperp, iR, vT_on_c, omega_b, Omega_ce, P_loc, theta0_loc, N0_loc, sigma, E2_loc):
    ...
    rho = np.sin(theta0_loc) * N0_loc * omega_b\
        * np.transpose(lorentz) * vperp * vT_on_c / Omega_ce
...
N_in = compute_N(theta_in, P_in, Omega_ce_in, omega_b)
...
R_res_max = np.sqrt((Npar_in*R_in)**2 +
                    (harmonic * Omega_ce0 * R0 / omega_b)**2)
...
for iR in range(Nr-2, -1, -1):
    ...
    if R_loc < max(R_res_max, R_eff_max) and R_loc > R_eff_min:
        print(f"iR={iR} is being calculated")
        Ne_loc = vec_Ne[iR]
        Te_loc = vec_Te[iR]
        B_loc = B0 * R0 / R_loc
        ...
        if (math.ceil(harmonic0) == harmonic):
            ...
            Pn_loc = np.pi*math.factorial(2*harmonic+1) / (2**harmonic*math.factorial(harmonic))**2 * \
                (harmonic * Omega_ce_loc / (omega_b * Nperp_loc))**2 * \
                (Big_A_loc + Big_B_loc)
            ...
            Power_absorbed *= Ne_loc * mass * dVpar * \
                dVperp * dR * R_loc * np.sqrt(2) * np.pi * W0
