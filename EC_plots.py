...
ax1.set_xlabel(r"$v_{\parallel}$", fontsize=20)
ax1.set_ylabel(r"$v_{\perp}$", fontsize=20)
ax1.set_title(r"$D_{n}/(v_{Te}^2 \Omega_{ce})$", fontsize=20)
ax1.set_aspect('equal', 'box')
plt.colorbar()

fig1.show()

print('P_{abs,tot}^{mod} / P_{abs,tot}^{ana}',
      (vec_Power[-1] - vec_Power[1])/(vec_Albajar[-1]-vec_Albajar[1]))
